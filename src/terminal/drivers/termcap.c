...
static int tcapcres(const char *);
...
static void tcapopen(void)
{
	...
	if (!term_init_ok) {
        ...
        safe_snprintf(err_str, sizeof(err_str), "Unknown terminal type %s!",
				tv_stype);
			puts(err_str);
			exit(1);
		}
        ...
        if ((term.t_nrow <= 0)
            ...
            mlwrite("termcap entry incomplete (lines)");
            exit(1);
        }

        if ((term.t_ncol <= 0)
            && (term.t_ncol = (short) tgetnum("co")) == -1) {
            mlwrite("Termcap entry incomplete (columns)");
            exit(1);
        }
#ifdef SIGWINCH
		term.t_mrow = MAXROW;
        ...
        if (p >= &tcapbuf[TCAPSLEN]) {
            mlwrite("Terminal description too big!");
            exit(1);
        }
#if PKCODE && USE_BROKEN_OPTIMIZATION
		term_init_ok = 1;
...
{
    // Block only SIGWINCH during cursor movement to avoid tearing
...
static void tcapeeol(void)
{
    // Block only SIGWINCH during EOL erase
    sigset_t oldmask, mask;
    sigemptyset(&mask);
#ifdef SIGWINCH
    sigaddset(&mask, SIGWINCH);
...
static void tcapeeop(void)
{
    // Block only SIGWINCH during EOP erase
    sigset_t oldmask, mask;
    sigemptyset(&mask);
#ifdef SIGWINCH
    sigaddset(&mask, SIGWINCH);
...
{
    // Block only SIGWINCH during attribute toggles
    sigset_t oldmask, mask;
    sigemptyset(&mask);
#ifdef SIGWINCH
    sigaddset(&mask, SIGWINCH);
	...
	if (state) {
		// Send bold ANSI sequence atomically for modern terminals
		// This ensures bold + reverse video for status line
		ttputc('\033');
		ttputc('[');
		ttputc('1');
		ttputc('m');
