...
static void print_performance_stats(const char *test_name, double start_time, double end_time, 
    ...
    if (data_size > 0) {
        LOG_INFOF("%s Performance: %.2fms, %.0f ops/sec, %.2f MB/sec", test_name, duration, ops_per_sec, mb_per_sec);
    } else {
        LOG_INFOF("%s Performance: %.2fms, %.0f ops/sec", test_name, duration, ops_per_sec);
    }
}

int test_large_file_operations(void) {
    LOG_INFOF("\n%s=== Testing Large File Operations ===%s", CYAN, RESET);
    int passed = 0, total = 0;
    
    // Test 1: Large file creation and writing
    total++;
    LOG_INFO("Testing large file creation and writing...");
    ...
    if (f) {
        ...
        if (chunk) {
            ...
            if (written == file_size) {
                ...
                LOG_INFOF("[SUCCESS] Large file creation: %zu bytes written", written);
                passed++;
            } else {
                LOG_ERRORF("[FAIL] Large file write incomplete: %zu/%zu bytes", written, file_size);
            }
        } else {
            LOG_ERROR("[FAIL] Failed to allocate write chunk");
            fclose(f);
        }
    } else {
        LOG_ERROR("[FAIL] Failed to create large test file");
    }
    
    // Test 2: Large file reading and processing
    total++;
    LOG_INFO("Testing large file reading and processing...");
    ...
    if (f) {
        ...
        if (read_chunk) {
            ...
            if (total_read > 0) {
                ...
                LOG_INFOF("[SUCCESS] Large file read: %zu bytes, %zu lines", total_read, lines_counted);
                passed++;
            } else {
                LOG_ERROR("[FAIL] Large file read failed");
            }
        } else {
            LOG_ERROR("[FAIL] Failed to allocate read chunk");
            fclose(f);
        }
    } else {
    ...
    LOG_INFOF("Large file operation tests: %d/%d passed", passed, total);
    return passed == total ? 0 : 1;
}

int test_memory_intensive_operations(void) {
    LOG_INFOF("\n%s=== Testing Memory Intensive Operations ===%s", CYAN, RESET);
    int passed = 0, total = 0;
    
    // Test 1: Multiple buffer allocation stress
    total++;
    LOG_INFO("Testing multiple buffer allocation stress...");
    ...
    if (buffers) {
        ...
        LOG_INFOF("Memory allocation: %.2fms, access: %.2fms, deallocation: %.2fms", mid_time - start_time, end_time - mid_time, end_time - mid_time);
        
        if (allocated >= buffer_count * 0.8) { // Allow some allocation failures
            LOG_INFOF("[SUCCESS] Memory stress test: %d/%d buffers allocated (checksum: %d)", allocated, buffer_count, checksum);
            passed++;
        } else {
            LOG_ERRORF("[FAIL] Memory stress test: only %d/%d buffers allocated", allocated, buffer_count);
        }
    } else {
        LOG_ERROR("[FAIL] Failed to allocate buffer array");
    }
    
    // Test 2: Memory fragmentation stress
    total++;
    LOG_INFO("Testing memory fragmentation stress...");
    ...
    if (frag_ptrs) {
        ...
        if (frag_allocated >= frag_iterations * 0.9) {
            LOG_INFOF("[SUCCESS] Fragmentation stress: %d/%d allocations", frag_allocated, frag_iterations);
            passed++;
        } else {
            LOG_ERRORF("[FAIL] Fragmentation stress failed: %d/%d allocations", frag_allocated, frag_iterations);
        }
    } else {
        LOG_ERROR("[FAIL] Failed to allocate fragmentation test array");
    }
    
    LOG_INFOF("Memory intensive operation tests: %d/%d passed", passed, total);
    return passed == total ? 0 : 1;
}

int test_rapid_ui_updates(void) {
    LOG_INFOF("\n%s=== Testing Rapid UI Updates ===%s", CYAN, RESET);
    int passed = 0, total = 0;
    
    // Test 1: Rapid screen refresh simulation
    total++;
    LOG_INFO("Testing rapid screen refresh simulation...");
    ...
    if (screen_buffer) {
        ...
        LOG_INFOF("[SUCCESS] Rapid UI updates: %d refreshes completed", refresh_count);
        passed++;
    } else {
        LOG_ERROR("[FAIL] Failed to allocate screen buffer");
    }
    
    // Test 2: Damage tracking simulation
    total++;
    LOG_INFO("Testing damage tracking simulation...");
    ...
    if (damage_rects) {
        ...
        LOG_INFOF("[SUCCESS] Damage tracking: %d rects consolidated to %d", damage_updates, consolidated);
        passed++;
    } else {
        LOG_ERROR("[FAIL] Failed to allocate damage rectangles");
    }
    
    LOG_INFOF("Rapid UI update tests: %d/%d passed", passed, total);
    return passed == total ? 0 : 1;
}
...
int test_concurrent_buffer_operations(void) {
    LOG_INFOF("\n%s=== Testing Concurrent Buffer Operations ===%s", CYAN, RESET);
    int passed = 0, total = 0;
    
    // Test 1: Multi-threaded buffer access
    total++;
    LOG_INFO("Testing multi-threaded buffer access...");
    ...
    if (shared_buffer) {
        ...
        if (shared_counter == thread_count * ops_per_thread) {
            LOG_INFOF("[SUCCESS] Concurrent operations: %d total operations", shared_counter);
            passed++;
        } else {
            LOG_ERRORF("[FAIL] Concurrent operations mismatch: %d expected %d", shared_counter, thread_count * ops_per_thread);
        }
        
        free(shared_buffer);
        pthread_mutex_destroy(&mutex);
    } else {
        LOG_ERROR("[FAIL] Failed to allocate shared buffer");
    }
    
    LOG_INFOF("Concurrent buffer operation tests: %d/%d passed", passed, total);
    return passed == total ? 0 : 1;
}

int test_search_performance_stress(void) {
    LOG_INFOF("\n%s=== Testing Search Performance Stress ===%s", CYAN, RESET);
    int passed = 0, total = 0;
    
    // Test 1: Large text search performance
    total++;
    LOG_INFO("Testing large text search performance...");
    ...
    if (large_text) {
        ...
        if (found_count >= pattern_instances * search_iterations * 0.9) {
            LOG_INFOF("[SUCCESS] Search performance: %d patterns found", found_count);
            passed++;
        } else {
            LOG_ERRORF("[FAIL] Search performance: %d patterns found, expected ~%d", found_count, pattern_instances * search_iterations);
        }
        
        free(large_text);
    } else {
        LOG_ERROR("[FAIL] Failed to allocate large text buffer");
    }
    
    // Test 2: Regex performance simulation (simplified)
    total++;
    LOG_INFO("Testing regex performance simulation...");
    ...
    if (email_matches > 0) {
        LOG_INFOF("[SUCCESS] Regex performance: %d email matches found", email_matches);
        passed++;
    } else {
        LOG_ERROR("[FAIL] Regex performance: no matches found");
    }
    
    LOG_INFOF("Search performance stress tests: %d/%d passed", passed, total);
    return passed == total ? 0 : 1;
}

int test_undo_redo_stress(void) {
    LOG_INFOF("\n%s=== Testing Undo/Redo Stress ===%s", CYAN, RESET);
    int passed = 0, total = 0;
    
    // Test 1: Large undo stack performance
    total++;
    LOG_INFO("Testing large undo stack performance...");
    ...
    LOG_INFOF("Undo stack build: %.2fms, undo ops: %.2fms, redo ops: %.2fms", build_time - start_time, undo_time - build_time, end_time - undo_time);
    
    if (undo_count >= undo_operations * 0.9) {
        LOG_INFOF("[SUCCESS] Undo/redo stress: %d operations in stack", undo_count);
        passed++;
    } else {
        LOG_ERRORF("[FAIL] Undo/redo stress: only %d/%d operations stored", undo_count, undo_operations);
    }
    
    LOG_INFOF("Undo/redo stress tests: %d/%d passed", passed, total);
    return passed == total ? 0 : 1;
}

int test_syntax_highlighting_stress(void) {
    LOG_INFOF("\n%s=== Testing Syntax Highlighting Stress ===%s", CYAN, RESET);
    int passed = 0, total = 0;
    
    // Test 1: Large source code syntax analysis
    total++;
    LOG_INFO("Testing large source code syntax analysis...");
    ...
    if (source_code) {
        ...
        if (total_tokens > 0) {
            LOG_INFOF("[SUCCESS] Syntax analysis: %d tokens (%d keywords, %d identifiers, %d comments)", total_tokens, token_counts[TOKEN_KEYWORD], 
                   token_counts[TOKEN_IDENTIFIER], token_counts[TOKEN_COMMENT]);
            passed++;
        } else {
            LOG_ERROR("[FAIL] Syntax analysis: no tokens found");
        }
        
        free(source_code);
    } else {
        LOG_ERROR("[FAIL] Failed to allocate source code buffer");
    }
    
    LOG_INFOF("Syntax highlighting stress tests: %d/%d passed", passed, total);
    return passed == total ? 0 : 1;
}