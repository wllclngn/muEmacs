// tests/unit/test_text_boyer_moore.c - Unit tests for src/text/boyer_moore.c
...
static int expect_match(const unsigned char* text, int text_len,
    ...
    if (bm_init(&ctx, (const unsigned char*)pat, (int)strlen(pat), cs) != 0) {
        LOG_ERRORF("[FAIL] bm_init failed for pattern '%s'", pat);
        return 0;
    }
    int got = bm_search(&ctx, text, text_len, start);
    if (got != want) {
        LOG_ERRORF("[FAIL] bm_search '%s' got %d want %d", pat, got, want);
        return 0;
    }
    return 1;
}
...
static int expect_rmatch(const unsigned char* text, int text_len,
    ...
    struct boyer_moore_context ctx;
    if (bm_init(&ctx, (const unsigned char*)pat, (int)strlen(pat), cs) != 0) {
        LOG_ERRORF("[FAIL] bm_init failed for pattern '%s' (reverse)", pat);
        return 0;
    }
    int got = bm_search_reverse(&ctx, text, text_len, start);
    if (got != want) {
        LOG_ERRORF("[FAIL] bm_search_reverse '%s' got %d want %d", pat, got, want);
        return 0;
    }
    return 1;
}
...
int test_bmh_edge_cases(void) {
    ...
    if (bm_init(&ctx, (const unsigned char*)"", 0, true) == 0) {
        LOG_ERROR("[FAIL] bm_init unexpectedly succeeded for empty pattern");
        ok = 0;
    }
    ...
    // start_pos beyond end → not found
    {
        ...
        if (got != -1) {
            LOG_ERRORF("[FAIL] start_pos==n expected -1, got %d", got);
            ok = 0;
        }
    }
...
int test_bmh_additional_edges(void) {
    ...
    // Pattern length equals text length
    {
        ...
        if (got != 0) { LOG_ERRORF("[FAIL] len==len cs match expected 0, got %d", got); ok = 0; }
        ...
        if (got != 0) { LOG_ERRORF("[FAIL] len==len ci match expected 0, got %d", got); ok = 0; }
    }
    ...
    // start_pos negative / beyond end handling (API guards)
    {
        ...
        if (got != -1) { LOG_ERRORF("[FAIL] start_pos<0 should yield -1, got %d", got); ok = 0; }
        got = bm_search(&ctx, t, n, 4); // n-m = 3; start beyond first window
        if (got != -1) { LOG_ERRORF("[FAIL] start_pos>n-m should yield -1, got %d", got); ok = 0; }
    }
    ...
    // Reverse search boundary: last <= start_pos
    {
        ...
        if (got != 7) { LOG_ERRORF("[FAIL] reverse from end expected 7, got %d", got); ok = 0; }
        got = bm_search_reverse(&ctx, t, n, 4); // before second match window >=7
        if (got != 2) { LOG_ERRORF("[FAIL] reverse from 4 expected 2, got %d", got); ok = 0; }
        got = bm_search_reverse(&ctx, t, n, 1); // before first match ends
        if (got != -1) { LOG_ERRORF("[FAIL] reverse from 1 expected -1, got %d", got); ok = 0; }
    }
    ...
    // UTF-8 multibyte boundaries: match single multibyte codepoint and across
    {
        ...
        int got = bm_search(&ctx, t, n, 0);
        if (got != 2) { LOG_ERRORF("[FAIL] UTF-8 single codepoint expected 2, got %d", got); ok = 0; }
        // Match spanning üß (4 bytes)
        const unsigned char* uesz = (const unsigned char*)"\xC3\xBC\xC3\x9F";
        bm_init(&ctx, uesz, 4, true);
        got = bm_search(&ctx, t, n, 0);
        if (got != 2) { LOG_ERRORF("[FAIL] UTF-8 span expected 2, got %d", got); ok = 0; }
    }
